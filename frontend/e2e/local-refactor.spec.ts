...
test('strategy workflow uses the toolbar publishing flow', async ({ page }) => {
  ...
  await expect(page.getByText('项目数据刷新', { exact: true })).toHaveCount(0)
  await expect(page.getByText('项目闭环刷新周期（分钟）', { exact: true })).toBeVisible()
  await expect(page.getByText('页面时区', { exact: true })).toHaveCount(0)
  await expect(page.locator('.loop-check-panel')).toHaveCount(0)
})
...
test('realtime closure uses the shared automatic-strategy rule layout', async ({ page }) => {
  ...
  const settings = workspace.getByRole('region', { name: '实时闭环设置' })
  await expect(settings.getByText('项目数据刷新', { exact: true })).toHaveCount(0)
  await expect(settings.getByText('该周期覆盖账户、百度报表、好多粉、预算、创意和归因等闭环数据', { exact: true })).toHaveCount(0)
  await expect(settings.getByText('页面时区', { exact: true })).toHaveCount(0)
  await expect(settings.getByText('项目闭环刷新周期（分钟）', { exact: true })).toBeVisible()
  await expect(settings.getByText('默认 60 分钟，允许 15–1440 分钟；从每个整数点开始计算', { exact: true })).toBeVisible()
  await expect(settings.getByText('运行与安全状态', { exact: true })).toHaveCount(0)
  await expect(settings.getByText('闭环运行状态', { exact: true })).toHaveCount(0)
  await expect(settings.getByText('规则评估', { exact: true })).toHaveCount(0)
...
test('project management is the root entry and project workspaces keep their own URL', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.locator('.nav-label')).toHaveText(['项目与账户', '系统设置'])
  await expect(page.locator('.nav-item')).toHaveText(['账户管理', '账户列表', '自动上线', '数据报表', '财务报表', '自动策略', '成员管理'])
  ...
  await expect(runtimeWidget.locator('.runtime-closure-column > .runtime-job-list-title')).toHaveText('实时闭环')
  await expect(runtimeWidget.locator('.runtime-closure-step strong')).toHaveText(['百度日报同步', '好多粉同步', '账户预算与余额', '创意审核', '账户归因'])
  ...
  expect(runtimeWidgetWidth).toBeGreaterThan(gridWidth * 0.49)
  expect(runtimeWidgetWidth).toBeLessThan(gridWidth * 0.52)
...
test('home four-column tiles fill the widescreen content area', async ({ page }) => {
  await page.setViewportSize({ width: 1920, height: 900 })
  await page.goto('/')

  const grid = page.locator('.home-dashboard-grid')
  const projectTile = page.getByRole('region', { name: '项目管理' })
  const runtimeTile = page.getByRole('region', { name: '运行与安全状态' })
  const codeTile = page.getByRole('region', { name: '代码同步' })
  await expect(codeTile).toBeVisible()

  const [gridBox, projectBox, runtimeBox, codeBox] = await Promise.all([
    grid.boundingBox(), projectTile.boundingBox(), runtimeTile.boundingBox(), codeTile.boundingBox(),
  ])
  expect(gridBox).not.toBeNull()
  expect(projectBox).not.toBeNull()
  expect(runtimeBox).not.toBeNull()
  expect(codeBox).not.toBeNull()
  expect(projectBox!.width).toBeGreaterThan(gridBox!.width * .49)
  expect(projectBox!.width).toBeLessThan(gridBox!.width * .52)
  expect(runtimeBox!.width).toBeGreaterThan(gridBox!.width * .49)
  expect(runtimeBox!.width).toBeLessThan(gridBox!.width * .52)
  expect(Math.abs(projectBox!.y - runtimeBox!.y)).toBeLessThanOrEqual(1)
  expect(codeBox!.width).toBeGreaterThan(gridBox!.width * .23)
  expect(codeBox!.width).toBeLessThan(gridBox!.width * .26)
  expect(codeBox!.y).toBeGreaterThan(projectBox!.y + projectBox!.height)
})
